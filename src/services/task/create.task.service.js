...
* Description : This file contains create operation releated functions for task schema
**/
...
module.exports = class createTaskService {
	...
	* Description  : This function stores a record in task table
	**/
		...
		try {
			...
			if (config.IsMockEnabled == true) { return taskMock[apiID] }
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_TASK_CREATED_SUCCESSFULLY);
				}
				else {
